...
mpl.use('Agg')  # Use 'Agg' for PNGs, or 'SVG' for SVG files
...
class ImageProcessor(object):
    ...
    def create_image_pyramids(self):
        ...
        # a, self.curr_cam0_pyramid = cv2.buildOpticalFlowPyramid(
        #     curr_cam0_img, self.config.win_size, self.config.pyramid_levels, 
        #     withDerivatives=False, pyrBorder=cv2.BORDER_REFLECT_101, derivBorder=cv2.BORDER_CONSTANT, tryReuseInputImage=False)
        # print(type(self.curr_cam0_pyramid))
        self.curr_cam0_pyramid = curr_cam0_img

        curr_cam1_img = self.cam1_curr_img_msg.image
        # b, self.curr_cam1_pyramid = cv2.buildOpticalFlowPyramid(
    ...
    def track_features(self):
        ...
        self.count_inlier0.append(len(cam0_ransac_inliers))
        self.count_inlier1.append(len(cam1_ransac_inliers))
        # self.plot_inliers_trend()
    ...
    def plot_inliers_trend(self):
        """
        Plots the trend of inliers over time for cam0 and cam1.
        """
        plt.figure(figsize=(10, 5))
        
        # Plot for cam0
        plt.plot(self.count_inlier0, label='Cam0 Inliers', color='blue', linestyle='-', marker='o')
        
        # Plot for cam1
        # plt.plot(self.count_inlier1, label='Cam1 Inliers', color='red', linestyle='--', marker='x')
        
        # Adding labels and title
        plt.xlabel('Time Step')
        plt.ylabel('Number of Inliers')
        plt.title('Trend of Inliers Over Time (With RANSAC)')
        plt.legend()
        plt.savefig("C:/Users/ashwi/Documents/CMU/FALL_2024/SLAM/SLAM_Project/msckf_vio_python-main/msckf_vio_python-main/inlier/abc.png")
        # Show the plot
        # plt.show()
        plt.close()
    ...
    def prune_features(self):
        ...
        for i, features in enumerate(self.curr_features):
            ...
            self.curr_features[i] = sorted(features, key=lambda x:x.lifetime, 
                reverse=True)[:self.config.grid_max_feature_num]
            self.prune_count = self.prune_count + 1
    ...
    def two_point_ransac(self, pts1, pts2, R_p_c, intrinsics, 
        ...
        # print(pts_diff)
        # print(pts1_undistorted.shape)
        # print(pts2_undistorted.shape)
        
        #!!!!!!!!!!!!!!!!!!!!!! 
        #Edited the indexing of undistroted points
        coeff_t = []
        for i, pt_diff in enumerate(pts_diff):
