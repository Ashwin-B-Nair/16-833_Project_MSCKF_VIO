...
class DataPublisher(object):
    def __init__(self, dataset, out_queue, duration=float('inf'), ratio=1.): 
        ...
        self.publish_thread = Thread(target=self.publish)   # Thread: (independent units of execution)
        
    def start(self, starttime):
        self.started = True
        self.starttime = starttime
        self.publish_thread.start() # This starts the thread and runs the publish method concurrently in the background.

        # When you create and start a new thread like this, it allows the publish method to run concurrently in the 
        # background while your main program can continue doing other things.
    ...
    def publish(self):
        ...
        while not self.stopped:
            ...
            if interval <= self.duration + 1e-3:    # Keep adding data to the output queue, in other words publishing data
...
if __name__ == '__main__':
    ...
    path = r"C:\Users\athar\OneDrive\Documents\Sem-1\SLAM\Project\codes\MH_01_easy"
