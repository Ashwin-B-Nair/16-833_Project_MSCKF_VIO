...
class IMUState(object):
    ...
    T_imu_body = Isometry3d(np.identity(3, dtype=np.float64), np.zeros(3, dtype=np.float64))
    ...
    def __init__(self, new_id=None):
        ...
        self.orientation = np.array([0.603879,-0.088957,-0.791736,-0.023906], dtype=np.float64)          # np.array([0., 0., 0., 1.], dtype=np.float64)
        # Position of the IMU (body) frame in the world frame.
        self.position = np.array([4.688319,-1.786938,0.783338],dtype=np.float64)     # np.zeros(3, dtype=np.float64)
        # Velocity of the IMU (body) frame in the world frame.
        self.velocity = np.array([-0.027876,0.033207,0.800006],dtype=np.float64)                # np.zeros(3, dtype=np.float64)

        # Bias for measured angular velocity and acceleration.
        self.gyro_bias = np.array([-0.003172,0.021267,0.078502], dtype = np.float64)               # np.zeros(3, dtype=np.float64)
        self.acc_bias = np.array([-0.025266,0.136696,0.075593], dtype = np.float64)                #   np.zeros(3, dtype=np.float64)
        ...
        self.orientation_null = np.array([0.603879,-0.088957,-0.791736,-0.023906], dtype=np.float64)        # np.array([0., 0., 0., 1.], dtype=np.float64)
        self.position_null =  np.array([4.688319,-1.786938,0.783338],dtype=np.float64)          # np.zeros(3, dtype=np.float64)
        self.velocity_null =  np.array([-0.027876,0.033207,0.800006],dtype=np.float64)          # np.zeros(3, dtype=np.float64)

        # Transformation between the IMU and the left camera (cam0)
        self.R_imu_cam0 = np.identity(3, dtype=np.float64)
        self.t_cam0_imu = np.zeros(3, dtype=np.float64)
...
class CAMState(object):
    ...
    def __init__(self, new_id=None):
        ...
        self.orientation = np.array([0.603879,-0.088957,-0.791736,-0.023906], dtype=np.float64)

        # Position of the camera frame in the world frame.
        # self.position = np.zeros(3, dtype=np.float64)
        self.position = np.array([4.688319,-1.786938,0.783338],dtype=np.float64)
        ...
        # have proper null space.
        # self.orientation_null = np.array([0., 0., 0., 1.], dtype=np.float64)
        self.orientation_null = np.array([0.534108,-0.153029,-0.827383,-0.082152], dtype=np.float64)
        # self.position_null = np.zeros(3, dtype=np.float64)
        self.position_null = np.array([4.688319,-1.786938,0.783338],dtype=np.float64)
...
class StateServer(object):
    ...
    def __init__(self):
        ...
        self.state_cov = np.zeros((21, 21), dtype=np.float64)
        self.continuous_noise_cov = np.zeros((12, 12), dtype=np.float64)
...
class MSCKF(object):
    def __init__(self, config):
        ...
        continuous_noise_cov = np.identity(12, dtype=np.float64)
    ...
    def imu_callback(self, imu_msg):
        ...
        # print("imu message buffer length", len(self.imu_msg_buffer))

        if not self.is_gravity_set:
            if len(self.imu_msg_buffer) >= 20:
                self.initialize_gravity_and_bias()
                self.is_gravity_set = True
                # print('set gravity to true')
    ...
    def process_model(self, time, m_gyro, m_acc):
        ...
        F = np.zeros((21,21), dtype=np.float64)
        G = np.zeros((21,12), dtype=np.float64)

        F[:3, :3] = -skew(gyro)
        F[:3,3:6] = -np.eye(3, dtype=np.float64)
        F[6:9,:3] = -to_rotation(imu_state.orientation).T @ skew(acc)
        F[6:9,9:12] = -to_rotation(imu_state.orientation).T
        F[12:15,6:9] = np.eye(3, dtype=np.float64)

        G[:3, :3] = -np.eye(3, dtype=np.float64)
        G[3:6, 3:6] = np.eye(3, dtype=np.float64)
        G[6:9, 6:9] = -to_rotation(imu_state.orientation).T
        G[9:12, 9:12] = np.eye(3, dtype=np.float64)        
        ...
        # print("imu_state.velocity_null: ", imu_state.velocity_null)
        # print("imu_state.velocity", imu_state.velocity)
        # print("IMUState.gravity", IMUState.gravity)
        ...
        # print("shape of self.state_server.continuous_noise_cov", np.shape(self.state_server.continuous_noise_cov))
        ...
        # print("shape of self.state_server.state_cov", np.shape(self.state_server.state_cov))
    ...
    def predict_new_state(self, dt, gyro, acc):
        ...
        # print("shape of gyro: ", np.shape(gyro))        # (3,) shape so there's no need to transpose
    ...
    def state_augmentation(self, time):
        ...
        # To simplify computation, the matrix J below is the nontrivial block (J_I)
        # Appendix B of "MSCKF" paper.
        J = np.zeros((6,21), dtype=np.float64)    # Shape (6,21)
        J[:3, :3] = R_i_c
        J[:3,15:18] = np.eye(3, dtype=np.float64)
        J[3:6, :3] = skew(R_w_i.T @ t_c_i)
        J[3:6, 12:15] = np.eye(3, dtype=np.float64) 
        J[3:6, 18:21] = R_w_i.T

        # Resize the state covariance matrix.
        old_rows, _ = self.state_server.state_cov.shape         # (21,21) initially, both dimensions keep increasing by 6 for each new camera state
        ...
        P12 = self.state_server.state_cov[:21 , 21:]           # Shape (21,6N)

        # P11_12 = np.hstack((P11 , P12))

        # J_final = np.zeros((6, old_rows))
        # J_final[: , :21] = J    

        # Fill in the augmented state covariance.
        # r1 = J @ P11_12       # Shape (6,21)

        r1 = np.hstack((J @ P11 , J @ P12))
    ...
    def publish(self, time):
        ...
        # print("returned rotation:", R_w_c.T)
        # print("returned rotation 2", T_c_w.matrix())
        # print(T_c_w.rotation)
